...
1. *(Recall — Mechanism)* The chapter claims one mechanism powers all three influences in G13. State it in one sentence. Then explain why the loop's force is not in any single component — what specifically happens when the loop closes on a short cycle that does not happen when any one component runs in isolation?
