...
G19 collects two influences: problem-solving teaching (d = 0.68) and teacher expectations (d = 0.43). They look like an odd pair until you see what the dataset saw: both rows are *calibration problems*.
...
1. *(Recall — Two calibration problems)* The chapter claims both influences in G19 are calibration problems. Explain what that means for each row: what is being calibrated in problem-solving teaching, what is being calibrated in teacher expectations, and why premature certainty destroys each mechanism.
